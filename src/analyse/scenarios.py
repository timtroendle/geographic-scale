...
def plot_costs(paths_to_results, path_to_costs):
    ...
    sns.heatmap(cost_heatmap(results, "national"), cmap=pal, ax=ax2, cbar=False)
    plt.ylabel("")
    plt.yticks([])
    plt.title("NATIONAL AUTARKY")
    ax3 = fig.add_subplot(gs[2])
    sns.heatmap(cost_heatmap(results, "continental"), cmap=pal, ax=ax3, cbar=False)
    plt.ylabel("")
    plt.yticks([])
...
def read_results(paths_to_results):
    ...
    results.fillna(0, inplace=True) # FIXME only because scenarios are missing
